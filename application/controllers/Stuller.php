 	...
 	public function cartadd(){
		...
		if($flag == 1){
					...
					*/
					...
					if(!empty($stuller_data["Stones"]))
					{
						...
						foreach($stuller_data["Stones"] as $key => $val)
						{
							...
							if (array_key_exists("Product",$val))
							{
								$stoneData = "";
								if($val["Product"]["Id"] != ""){
						...
						if (array_key_exists("Diamond",$val))
						{
							$stoneData = "";
			...
			else
			{
				...
				//echo "<pre>";print_r($stuller_data);exit;
				//Images
				if(count($stuller_images_arr) > 0)
				{
					foreach($stuller_images_arr as $key => $value)
				...
				$tags = implode(",",$arrCollection);
				$stock_weight = $stuller_data["Products"][0]["Weight"];
