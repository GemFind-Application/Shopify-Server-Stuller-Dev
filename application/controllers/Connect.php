...
class Connect extends CI_Controller {
	...
	{		
		...
		if($_SERVER['HTTP_SEC_FETCH_DEST'] == 'iframe'){
			...
			if (hash_equals($hmac, $computed_hmac)) {
			  ...
			  if(isset($_REQUEST['SubmitStullerSetting']))
			  {
					...
					$adminOptionData = array(
					  ...
					  'enable_popup_message'  => $this->input->post('enable_popup_message'),
		    ...
		    $redirectURL = 'https://'.$shop.'/admin/apps/gemfind-stuller-dev';
		    $this->registerShopifyAppUninstallWebhookStuller($shop,$access_token);
		    $this->installApplication($shop,$access_token);
	...
	public function installApplication($shop,$shop_access_token){
		...
		$admin_subject = $data->name . " : GemFind Stuller App Install";
    ...
    // // file_put_contents($file, $post_json);
    // $endpoint = 'https://api.hubapi.com/contacts/v1/contact?hapikey=ee625d9a-7fde-44b5-b026-d5f771cfc343';
    // $ch = curl_init();
    // curl_setopt($ch, CURLOPT_POST, true);
    // curl_setopt($ch, CURLOPT_POSTFIELDS, $post_json);
    // curl_setopt($ch, CURLOPT_URL, $endpoint);
	...
	public function installhubspotStuller($templateValueReplacement){
		...
		// $now->setTimezone(new DateTimezone('Asia/Kolkata'));
		// $current_date = $now->format('Y-m-d H:i:s');
		// $domain = $this->input->post('shop');
		// $file = "domain_name.txt";
  ...
  //       if ($status_code == 200 && $result['status']!= "error") {
  //           $arr1 = array(
  //               'properties' => array(
  //                   array(
  //                       'property' => 'email',
  ...
  //               	),
  //               	array(
	 //                    'property' => 'ShopifyStuller_Service_Fee',
  ...
  //           );
  //           $post_json1 = json_encode($arr1);
  //           $email_id1=$this->input->post('email');
  //           $endpoint1 ='https://api.hubapi.com/contacts/v1/contact/email/'.$email_id1.'/profile?hapikey=ee625d9a-7fde-44b5-b026-d5f771cfc343';
